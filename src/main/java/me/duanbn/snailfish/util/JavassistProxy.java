...
public class JavassistProxy {
	...
	private static String buildCacheKey(Class<?> interfaClass, InvocationHandler handler) {
		return interfaClass.getName() + "_" + handler.toString();
	}

	/**
	 * 暴露给用户的动态代理接口，返回某个接口的动态代理对象.
	 * 
	 * @param interfaceClassName              String 要动态代理的接口类名
	...
	public static Object newProxyInstance(Class<?> interfaceClass, InvocationHandler interceptorHandler)
		...
		synchronized (lock) {
			if (!cache.containsKey(interfaceClass)) {
				...
				cp.insertClassPath(new ClassClassPath(interfaceClass));
				cp.insertClassPath(new ClassClassPath(interceptorHandler.getClass()));
				...
				cache.put(buildCacheKey(interfaceClass, interceptorHandler), obj);
			}
		}
